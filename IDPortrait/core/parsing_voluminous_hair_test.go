...
func TestVoluminousHairNotRejectedAsHat(t *testing.T) {
	path := filepath.Join("testdata", "voluminous_hair_no_hat.png")
	f, err := os.Open(path)
	if err != nil {
		t.Skip(err)
	}
	...
	if strings.Contains(res.Reason, "帽子") {
		t.Fatalf("voluminous crown hair must not be hat reject, got %q", res.Reason)
	}
	if !res.OK {
